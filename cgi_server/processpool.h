    ...
    :m_listenfd(listenfd), m_process_number(process_number), m_idx(-1), m_stop(false)
{
    ...
    for(int i = 0; i < process_number; i++)
    {
        ...
        if(m_sub_process[i].m_pid > 0)      //父进程关闭写端 
        {
...
void processpool<T>::setup_sig_pipe()
{
    ...
    setnonblocking(sig_pipefd[1]);      //读端设置非阻塞
    addfd(m_epollfd, sig_pipefd[0]);    //监听树监听写端
...
/* 父进程中m_idx值为-1， 子进程中m_idx值大于等于0， 我们据此判断接下来要运行的是父进程还是子进程 */
template<typename T>
...
void processpool<T>::run_child()
{
    ...
    while(!m_stop)
    {
        ...
        for(int i = 0; i < number; i++) 
        {
            ...
            {
                int client = 0;
                /* 从父、子进程至之间的管道读取数据， 并将结果保存在变量client中， 如果读取成功表示有新的客户连接到来 */
    ...
    close(pipefd);
    //close(m_listenfd);
    /* 我们将这句话注释掉，以提醒读者；应该由m_listenfd的创建者来关闭这个文件描述符（见后文），即所谓的
       “对象（比如一个文件描述符， 又或者一段堆内存）由那个函数创建，就应该由那个函数销毁” */
    close(m_epollfd);
}
template<typename T>
    ...
    while(!m_stop)
    {
        ...
        for(int i = 0; i < number; i++) 
        {
            ...
            if(sockfd == m_listenfd)
            {
                ...
                send(m_sub_process[j].m_pipefd[1], (char *)&new_conn, sizeof(new_conn), 0);
                        ...
                        switch(signals[i])
                        {
                            case SIGCHLD:
                            {
                                ...
                                while((pid == waitpid(-1, &stat, WNOHANG)) > 0)
                                {
                                    ...
                                    {
                                        /* 如果进程池中的第i个子进程退出了，则主进程关闭相应的通信管道，并设置相应的m_pid为-1，
                                           以标记该子进程已经退出了 */
                            ...
                            case SIGINT:
                            {
                                /* 如果父进程接收到中之信号，那么就需要杀死所有的子进程，并等待他们全部结束。当然，
                                   通知子进程结束的更好的方法是向父、子进程之间的管道发送特殊数据，读者不妨自己实现 */
                                ...
                                for(int i = 0; i < m_process_number; ++i)
                                {
                                    ...
                                    if(pid != -1)
                                    {
                                        ...
                                        if(ret == -1)
                                        {
                                            perror("in prant process kill error:");
                                            break;
                                        }
                                    }
                                }
